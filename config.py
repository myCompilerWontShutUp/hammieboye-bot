...
# 관리자 콘솔("주인님 가라사대 ...")을 쓸 수 있는 최초 주인(prime)의 Discord user ID.
...
# FastMode(OpenAI service_tier="fast") 전역 on/off. 코드 수정 없이 .env만 바꾸고
# 재시작하면 즉시 롤백 가능. 관리자 자연어 설명 호출은 이 값과 무관하게 항상 미적용
# (responses/engine.py::get_admin_command_response 참고).
OPENAI_FAST_MODE = os.environ["OPENAI_FAST_MODE"].strip().lower() == "true"


def openai_service_tier_kwargs() -> dict:
    """responses.create(**이 결과)로 쓴다. 꺼져 있으면 파라미터 자체를 생략한다 —
    생략과 명시적 None이 SDK에서 항상 동일하게 처리된다는 보장이 없어서다."""

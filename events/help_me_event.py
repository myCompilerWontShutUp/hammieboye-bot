...
- relevant: Hammie가 언급한 상황/주제(목마름/심심함/필요한 것 등)에 어떤 식으로든 반응하는
  답장. **판정 기준은 오직 "그 상황/주제를 언급하거나 반응했는지"뿐이다 — 그 반응이 실제로
  문제를 완벽히 해결하는지, 논리적으로 말이 되는지, 말투나 감정 표현이 어떤지는 절대 판정에
  영향을 주면 안 된다.** 실제로 챙겨주는 행동("물통 채워줄게")은 당연히 relevant고, 그 상황을
  있는 그대로 받아주거나 다소 엉뚱하게 반응하는 말("물마셔"처럼 물이 없다는데도 마시라고
  하는 식)도 그 주제 자체에서 벗어나지만 않았다면 relevant다 — 완벽한 해결책일 필요는 없고
  주제와 관련만 있으면 충분하다. 힘들었다, 돈/시간이 많이 들었다는 하소연, 피곤함·
  시무룩함·투덜거림이 섞여 있어도 상관없다. 예: "케이지 대따 큰거 놓았어... 돈 다
  떨어졌어... (시무룩)"은 relevant다(돈이 떨어져 속상하다는 감정은 무시).
- negative: Hammie를 실제로 무시하거나, 쌀쌀맞게 거절하거나, "네가 알아서 해"/"싫어"처럼
  도와주는 행동 자체를 명확히 거부·회피하는 반응. 단순한 하소연이나 힘들다는 말투, 장난스럽게
  주제에 반응하는 말만으로는 절대 negative가 아니다 — 도와주지 않겠다는 의사가 명확할
  때만 negative다.
- irrelevant: Hammie가 언급한 상황/주제와 아예 무관한, 완전히 다른 화제의 일반 대화(예:
  "오늘 학교 어땠어?", "점심 뭐 먹지" 같은 진짜 딴 얘기). **그 주제를 조금이라도 언급했거나
  어떤 식으로든 반응했다면 절대 irrelevant가 아니다** — irrelevant는 정말 완전히 무관할
  때만 고른다.

애매하면 relevant로 판정한다 — negative와 irrelevant는 명확한 근거가 있을 때만 고른다.\
"""
...
# **2026-09-08 수정**: "물마셔"(물이 없다는 이벤트에 대한 반응)처럼 이벤트 주제를 조금이라도
# 언급/반응한 메시지까지 irrelevant로 잘못 판정돼 이 페널티+리다이렉트를 맞는 경우가
# 실사용에서 보고됐다 — 실제로 해결이 안 되는 반응이라도 주제 자체에서 벗어나지 않았으면
# irrelevant가 아니라 relevant로 판정하도록 `_RESPONSE_JUDGE_INSTRUCTIONS`를 완화했다
# (irrelevant는 이제 "완전히 무관한 딴 얘기"에만 적용). 라이브 API로 원 사례+여러 대조군
# (진짜 무관한 잡담/명확한 거절/진짜 도움/애매한 딴소리)을 반복 재현해 전부 기대한 대로
# 판정되는 것까지 확인.
...
async def _post_one(event: dict) -> None:
    ...
    for guild in _client.guilds:
        ...
        if channel is None:
            continue
        try:
            sent = await channel.send(event["prompt_text"], embed=embed)
...
async def handle_potential_response(
    ...
    - 다섯 번째 값(active_prompt_text)은 지금 실제로 활성 상태인 이벤트가 있을 때만
      ...
      반응하게 한다(2026-09-06, 재진술 안 하면 엉뚱하게 되묻으면서 보상만 지급되던
      문제 수정).

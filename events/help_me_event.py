...
- relevant: Hammie의 상황(목마름/심심함/필요한 것 등)에 맞게 실제로 챙겨주거나 도와주는
  반응. **판정 기준은 오직 "실제로 도와주는 행동을 했는지/하겠다고 했는지"뿐이다 — 말투나
  감정 표현은 절대 판정에 영향을 주면 안 된다.** 힘들었다, 돈/시간이 많이 들었다는 하소연,
  피곤함·시무룩함·투덜거림이 섞여 있어도 결국 도와준 행동이면 무조건 relevant다. 예:
  "케이지 대따 큰거 놓았어... 돈 다 떨어졌어... (시무룩)"은 실제로 케이지를 마련해준
  행동이 핵심이므로 relevant다(돈이 떨어져 속상하다는 감정은 무시).
- negative: Hammie를 실제로 무시하거나, 쌀쌀맞게 거절하거나, "네가 알아서 해"/"싫어"처럼
  도와주는 행동 자체를 명확히 거부·회피하는 반응. 단순한 하소연이나 힘들다는 말투만으로는
  절대 negative가 아니다 — 도와주지 않겠다는 의사가 명확할 때만 negative다.
- irrelevant: 이벤트와 아예 관련 없는 그냥 일반 대화

애매하면 relevant나 irrelevant 쪽으로 판정한다 — negative는 확실할 때만 고른다.\
"""

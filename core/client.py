...
# 실제 등록 이름과 정확히 일치해야 함). 2026-09-09 발견/수정: 舊 "가입"/"가입-수집항목"은
# 2026-09-08 이미 폐지·개명된 죽은 이름이라 이 예외가 실질적으로 /탈퇴에만 적용되고
# 있었다(신규 이름 "수집항목"이 세트에 없어서 §14-8에 문서화된 "/수집항목은 메인/서브
# 무관 항상 동작" 보장이 조용히 깨져 있었음) — 지금 이름들로 갱신.
# 2026-09-10 봇정보-레벨/봇정보-규칙 신규 추가 — 같은 "봇정보-" 접두 원칙(항상 어느
# 채널에서나 동작하는 순수 정보 명령어)을 그대로 적용한다.
_ONBOARDING_COMMAND_NAMES = {"봇정보-수집항목", "봇정보-확률공개", "봇정보-레벨", "봇정보-규칙", "탈퇴"}
